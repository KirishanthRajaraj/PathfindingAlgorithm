  ...
  this.calculate = function() {
    ...
    while (openSet.length != 0) {
      ...
      for (let i=0;i < neighbours.length; i++) {
        temp_tenative_g = currentNode.g + distNeighbour(currentNode, neighbours[i]);
        
        // avoid coloring start and endnode
        if(JSON.stringify(startingNode) !== JSON.stringify(neighbours[i]) && JSON.stringify(targetNode) !== JSON.stringify(neighbours[i])) {
          neighbours[i].color(color(209, 134, 000));
        }
...
function distNeighbour(current, neighbour) {
    return Math.sqrt(Math.pow(current.x - neighbour.x, 2) + Math.pow(current.y - neighbour.y, 2));
}
...
function colorFinalPath(finalPath, startingNode, targetNode) {
  for (let i = 0; i < finalPath.length; i++) {
    // avoid coloring start and endnode
    if(JSON.stringify(startingNode) !== JSON.stringify(finalPath[i]) && JSON.stringify(targetNode) !== JSON.stringify(finalPath[i])) {
            finalPath[i].color(color(0, 0, 255));
    }
  }
}
